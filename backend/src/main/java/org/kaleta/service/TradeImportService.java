...
public class TradeImportService
{
    ...
    private ParsedCsv parseCsv(String csv)
    {
        ...
        if (csv.length() > MAX_CSV_LENGTH) {
            throw new InvalidInputException("CSV file exceeds the maximum size of " + MAX_CSV_LENGTH + " characters");
        }

        List<CsvRecord> records = readCsvRecords(csv).stream()
        ...
        actualHeaders = actualHeaders.stream()
                ...
                .toList();
        if (!actualHeaders.equals(CSV_HEADERS)) {
            throw new InvalidInputException("CSV header must be: " + String.join(",", CSV_HEADERS));
        }
        ...
        for (CsvRecord record : records.subList(1, records.size())) {
            if (rows.size() == MAX_ROWS) {
                throw new InvalidInputException("CSV contains more than " + MAX_ROWS + " trade rows");
            }
        ...
        if (rows.isEmpty()) {
            throw new InvalidInputException("CSV contains no trade rows");
        }
        return new ParsedCsv(rows, errors);
    }

    private List<CsvRecord> readCsvRecords(String csv)
    {
        ...
        if (quoted) {
            throw new InvalidInputException("CSV contains an unclosed quoted value starting near line " + recordLineNumber);
        }
