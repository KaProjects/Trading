...
const selectFields = (rows, fields) => rows.map(row => Object.fromEntries(
...
export const CsvImportPage = ({
    ...
    columns,
    accentColor,
    loadingMessage = "Building import preview...",
}) => {
    ...
    async function loadPreview(file) {
        ...
        try {
            ...
            const response = await axios.post(backend + previewPath, csv, {
    ...
    async function importRows() {
        ...
        setAlert(null);
        try {
            const response = await axios.post(backend + importPath, {
                rows: selectFields(preview.rows, requestFields),
            });
            const importedRows = response.data?.rows?.length ?? preview.rows.length;
            setSuccess(
                `${importedRows} ${importedRows === 1 ? entity : entityPlural} imported successfully.`,
            );
        } catch (error) {
    ...
    const sectionTitle = entityPlural.charAt(0).toUpperCase() + entityPlural.slice(1);

    return (
        <Box sx={{maxWidth: 1280, margin: "0 auto", paddingTop: {xs: 1, md: 3}, paddingBottom: 4}}>
            <Paper elevation={3} sx={{overflow: "hidden", borderTop: "5px solid", borderTopColor: accentColor}}>
                <Box sx={{padding: {xs: 2, md: 3}, background: "linear-gradient(115deg, #fff8ec, #ffffff 60%)"}}>
                    ...
                    <Stack direction={{xs: "column", sm: "row"}} spacing={1.5} sx={{marginTop: 3}}>
                        <Button variant="outlined" startIcon={<ContentCopyIcon/>} onClick={copyTemplate}>
                    ...
                    <Box
                        role="button"
                        tabIndex={0}
                        aria-label={`Drop ${entity} CSV file`}
                        onClick={() => fileInput.current?.click()}
                        onKeyDown={event => {
                    ...
                    >
                        ...
                        <Typography sx={{fontWeight: 600}}>{fileName || "Drop a CSV file here"}</Typography>
                    ...
                    {preview?.rows?.length > 0 &&
                        <>
                            ...
                            >
                                ...
                                >
                                    {importing ? "Importing..." : `Import ${preview.rows.length} ${entityPlural}`}
                                </Button>
                            </Stack>

                            <TableContainer sx={{overflowX: "auto", border: "1px solid", borderColor: "divider", borderRadius: 1}}>
                                <Table size="small" stickyHeader aria-label={`${entity} import preview`}>
                                    <TableHead>
                                        <TableRow>
                                            {columns.map(column => (
